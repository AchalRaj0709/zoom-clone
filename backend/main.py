...
# Configure CORS for Next.js dev server on http://localhost:3000
app.add_middleware(
    CORSMiddleware,
...
# Auto-seed database during module load/startup
db = next(get_db())
try:
    ...
    if meeting_count == 0:
        ...
        for i, (title, duration) in enumerate(past_meetings):
            ...
            db_meeting = models.Meeting(
                ...
                scheduled_at=datetime.datetime.utcnow() - datetime.timedelta(days=i+1, hours=i),
                duration_minutes=duration,
                status="ended",
                created_at=datetime.datetime.utcnow() - datetime.timedelta(days=i+1, hours=i)
            )
            db.add(db_meeting)
            db.commit()
        ...
        for i, (title, duration, offset) in enumerate(upcoming_meetings):
            digits = f"11122233{i}"
            meeting_id = f"{digits[:3]}-{digits[3:6]}-{digits[6:]}"
            scheduled_time = datetime.datetime.utcnow() + offset
            db_meeting = models.Meeting(
                meeting_id=meeting_id,
                title=title,
